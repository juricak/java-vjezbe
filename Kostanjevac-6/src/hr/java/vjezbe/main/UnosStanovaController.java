...
public class UnosStanovaController {
	...
	public void spremiStan() {
		...
		if(naslov.isBlank()) {
			imaGresaka = true;
			greske.append("Naslov stana nije upisan!\n");
		}
		...
		if(opis.isBlank()) {
			imaGresaka = true;
			greske.append("Opis stana nije upisan!\n");
		}
		BigDecimal cijena = new BigDecimal(0);
		
		if(cijenaTextField.getText().isBlank()) {
			imaGresaka = true;
			greske.append("Cijena stana nije upisana!\n");
		}else {
		...
		if(kvadraturaTextField.getText().isBlank()) {
			imaGresaka = true;
			greske.append("Kvadratura stana nije upisana!\n");
		}else {
		...
		if(imaGresaka) {
			...
			alert.setContentText("Podaci o novom stanu su uspješno spremljeni!");
			alert.showAndWait();
			Stan noviStan = new Stan(naslov,opis,stanje,cijena, kvadratura);
			ProdajaController.listaArtikala.add(noviStan);
		}
