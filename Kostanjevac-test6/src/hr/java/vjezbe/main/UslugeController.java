...
public class UslugeController {
	...
	public static final List<Usluga> listaUsluga = new ArrayList<>();
    ...
    void initialize() {
		...
		List<Usluga> unosLista = ProdajaController.listaArtikala.stream().filter(p -> p instanceof Usluga).map(sc -> (Usluga)sc).collect(Collectors.toList());
		listaUsluga.addAll(unosLista);
		tablicaUsluga.setItems(FXCollections.observableArrayList(unosLista));
    ...
    private void stanoviPretrazivanje(ActionEvent event) {
    	...
    	filtriranaLista.addAll(listaUsluga);
